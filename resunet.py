# -*- coding: utf-8 -*-
"""ClimateChange_ResUNet.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1ypwp1RMYfzv53pQ2ZB6857Z2qH3mwUKg
"""
...
class ResidualBlock(nn.Module):
    ...
    This block applies two convolutional layers with batch normalization and
    a LeakyReLU activation function. It also includes a residual connection
    to facilitate gradient flow and improve training.

    If the input and output channels differ, a 1x1 convolution is used to
    adjust the residual connection.
    """
...
class ResidualUNet(nn.Module):
    """
    A U-Net implementation with residual blocks.
    ...
    """
    def __init__(self, c_in=3, c_out=3,base_channels=64, depth=4, dropout=0.0):
        super().__init__()

        self.down_channels = []
        for i in range(depth + 1):  # +1 because bottleneck
            self.down_channels.append(base_channels * (2**i))


        # Initial conv to go from c_in to first channel
        self.init_conv = ResidualBlock(in_c=c_in, out_c=self.down_channels[0], dropout=dropout)

        # Encoder
        # each downsample is stride=2
        ...
        # Bottleneck
        self.bottleneck = ResidualBlock(self.down_channels[-1], self.down_channels[-1], dropout=dropout)
...
def main_train(train_csv, test_csv, train_loader,test_loader,optimizer, model, num_epochs=10, batch_size=2, device='cuda'):
    """
    Trains the ResUNet model for a specified number of epochs.
    ...
    Args:
        ...
        model (nn.Module): The ResUNet model.
    ...
    Returns:
        nn.Module: The trained ResUNet model.
    """
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch+1) % 5 == 0:
          model_path = os.path.join(
              ...
              f"epoch_{epoch+1}_ResUnet_V2_2.pth"
          )
...
# Create an instance of the ResUNet model.
model = ResidualUNet(c_in=9, c_out=2, dropout=0.1,base_channels=128, depth=4).to(device)
